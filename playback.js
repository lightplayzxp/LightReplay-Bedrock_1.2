...
/**
 * Plays the camera path, every recorded track (one actor each, with its
 * own start-offset), and recorded world events (drops/door toggles)
 * together - all on one shared clock.
 *
 * Free Cam (Settings > Playback Options) skips the scripted camera path
 * entirely and just drops you into a free-fly camera you control
 * yourself, so you can watch all the actors from any angle without
 * having set up keyframes first.
 *
...
export function startPlayback(player, startTick) {
    ...
    try {
        ...
        // FIX: Calculate correct timeline duration accounting for track offsets
        ...
        // FIX: Pass each track's offsetTicks to ensure tracks start at correct time
...
/** Once every scheduled timeout AND every track has finished, end playback on its own. */
